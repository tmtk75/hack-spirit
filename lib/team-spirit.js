...
class TeamSpirit {
  ...
  * askForOvertime(date, note) {
    yield this.askFor(AttendanceApplicationType.Overtime, date, note);
  }

  * askForBeingDelayed(date, note, isPersonalReason) {
    yield this.askFor(AttendanceApplicationType.Delayed,
                      date, note, isPersonalReason);
  }

  * askFor(type, date, note, isPersonalReason) {
    let timeInputSuffix = '';
    ...
    yield this.needChangeMonth(date).then((needChangeMonth) => {
      ...
      return this.nightmare.click(`#empApplyDone${result.index}`);
    });
  }
  ...
  * recordBreakTime(startDate, endDate) {
    let dateString     = startDate.toFormat('YYYY-MM-DD');
