...
class HackSpirit {
  ...
  askForBeingDelayed(userName, password, date, note, isPersonal) {
    let self = this;
    let ts = self.teamSpiritClient;
    co(function *() {
      if (!userName || !password) {
      ...
      let cookies    = yield ts.login(userName, password);
      yield ts.askForBeingDelayed(date, note, isPersonal);
      let d = moment(date).format("YYYYMMDDHHmm");
      self.print(`Succeeded in asking for being delayed: ${d} : ${note}.` +
